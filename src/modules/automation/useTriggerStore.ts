...
export const useTriggerDryRunStore = defineStore('trigger-gry-run', () => {
  ...
  const dryRun = async (clubSlug: string) => {
    ...
    try {
      ...
      const processingTypeData = processingTypeOptionsMap[$trigger.processingType];
      if (processingTypeData.showOpts) processingOpts = JSON.parse($trigger.processingOptsInput) as Record<string, unknown>;
      if (processingTypeData.showCode) processingOpts.code = $trigger.processingCodeInput;
      ...
      const result = await api.post<{
        ...
        processingOpts,
        processingData: processingTypeData.showData ? JSON.parse($trigger.processingDataInput) as string : {},
...
export const useTriggerStore = defineStore('trigger', {
  ...
  actions: {
    ...
    async create(clubSlug: string) {
      ...
      try {
        let processingOpts: Record<string, unknown> = {};
        const processingTypeData = processingTypeOptionsMap[this.processingType];
        if (processingTypeData.showOpts) processingOpts = JSON.parse(this.processingOptsInput) as Record<string, unknown>;
        if (processingTypeData.showCode) processingOpts.code = this.processingCodeInput;
        ...
        const result = await api.post<{
          ...
          processingOpts,
          processingData: processingTypeData.showData ? JSON.parse(this.processingDataInput) as string : {},
