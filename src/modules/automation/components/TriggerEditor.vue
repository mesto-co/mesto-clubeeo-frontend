<template>
    ...
    <q-card-section>
      <div class='row q-col-gutter-md'>
        ...
        <div class='col-sm col-12'>
          <c-select
            label='event type'
            v-model='value.eventType' map-options emit-value
            :disable='!eventSource'
  ...
  <c-select
    ...
    class='q-mb-md'
    v-model='value.processingType' map-options emit-value
  ...
  <template v-if='processingTypeOption'>
    ...
    >
      <template v-slot:append>
        <q-btn flat dense round icon='fa-regular fa-circle-question' @click="isCodeHelpShown = true"/>
      </template>
    </q-input>
    ...
    <q-card-section>
      <div class='row q-col-gutter-md'>
        ...
        <div class='col-sm col-12'>
          <c-select
            label='task type'
            v-model='value.taskType' map-options emit-value
            :disable='!taskTarget'
...
import {
  eventSourceOptions,
  eventSourceOptionsMap, eventTypeOptionsMap,
  processingTypeOptions, processingTypeOptionsMap,
  taskTargetOptions, taskTargetOptionsMap, taskTypeOptionsMap
...
export default defineComponent({
  ...
  setup() {
    ...
    const eventTypeOptions = computed(
      () => (eventSource.value?.value ? eventSourceOptionsMap[eventSource.value?.value]?.children : []) || []
    );
    const pickEventSourceByType = () => {
      const eventTypeOption = eventTypeOptionsMap[value.eventType || ''];
      if (eventTypeOption?.parent?.value) eventSource.value = eventSourceOptionsMap[eventTypeOption?.parent?.value];
    }
    ...
    const taskTypeOptions = computed(
      () => (taskTarget.value?.value ? taskTargetOptionsMap[taskTarget.value?.value]?.children : []) || []
    );
    const pickTaskTargetByType = () => {
      const taskTypeOption = taskTypeOptionsMap[value.taskType || ''];
      if (taskTypeOption?.parent?.value) taskTarget.value = taskTargetOptionsMap[taskTypeOption?.parent?.value];
    }
    ...
    const processingTypeOption = computed(() => value?.processingType ? processingTypeOptionsMap[value.processingType] : null)
