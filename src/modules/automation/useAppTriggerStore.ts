...
export const useAppTriggerStore = defineStore('trigger', {
  ...
  getters: {
    apiData(state: ITrigger) {
      ...
      const processingTypeData = state.processingType
        ? processingTypeOptionsMap[state.processingType]
        : null;
      if (processingTypeData?.showOpts)
        processingOpts = JSON.parse(state.processingOptsInput) as Record<
          string,
          unknown
        >;
      if (processingTypeData?.showCode)
        processingOpts.code = state.processingCodeInput;
      ...
      return {
        ...
        processingData: processingTypeData?.showData
          ? (JSON.parse(state.processingDataInput) as string)
          : {},
      ...
      try {
        ...
        const trigger = {
          ...
          sourceApp: data.sourceApp
            ? { value: String(data.sourceApp), label: String(data.sourceApp) }
            : null,
          targetApp: data.targetApp
            ? { value: String(data.targetApp), label: String(data.targetApp) }
            : null,
          eventType: data.eventType
            ? { value: String(data.eventType), label: String(data.eventType) }
            : null,
