<template>
  ...
  >
    ...
    <template v-if="app">
      ...
      <div
        v-if="app && Object.keys(app.actions || {}).length > 0"
      ...
      <div
        v-if="app && Object.keys(app.events || {}).length > 0"
        class="row q-col-gutter-md q-py-sm"
      >
        <div class="col">
...
export default defineComponent({
  ...
  setup() {
    ...
    const load = async () => {
      ...
      const result = await api.get<{ clubApp: IClubApp }>(
      ...
      if (!app.value.coverImg)
        app.value.coverImg = `https://ui-avatars.com/api/?name=${app.value.name}&size=256&bold=true&format=svg&background=1D1D27&color=fff`;
