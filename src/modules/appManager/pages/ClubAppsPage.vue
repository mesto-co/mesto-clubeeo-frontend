<template>
  ...
  >
    ...
    <div v-if="!isLoading && appList.length === 0" class="text-center">
      <div class="q-pt-xl">you don't have any installed apps yet</div>
      <club-button
    ...
    <div class="flex flex-center q-my-md" v-if="!isLoading">
      <template v-for="(clubApp, i) of appList" v-bind:key="i">
        <app-card class="q-ma-sm" :app="mapToAppCard(clubApp)">
          <template v-slot:buttons>
            <q-btn
              ...
              :to="{
                ...
                params: { appSlug: clubApp.appSlug },
              }"
            />
          </template>
        </app-card>
...
export default defineComponent({
  ...
  setup() {
    ...
    const mapToAppCard = (clubApp: IClubApp) => {
      return {
        coverImg:
          clubApp.app?.coverImg ||
          `https://ui-avatars.com/api/?name=${clubApp.title}&size=256&bold=true&format=svg&background=1D1D27&color=fff`,
        name: clubApp.title,
