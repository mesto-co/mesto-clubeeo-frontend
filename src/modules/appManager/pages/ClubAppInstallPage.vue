<template>
  ...
  >
    <template v-if='app'>
      ...
      <div v-if='Object.keys(app.actions).length > 0'
      ...
      <div v-if='Object.keys(app.events).length > 0'
           class='row q-col-gutter-md q-py-sm'>
        <div class='col'>
...
import { IPropValue, IRegistryItem, serializeProp } from 'src/modules/appManager/shared/propHelpers';
...
export default defineComponent({
  ...
  setup() {
    ...
    const load = async () => {
      ...
      const result = await api.get<{ app: IRegistryItem }>(`/api/club/${clubSlug.value}/app/registry/${appKey.value}`);
