...
export const useClubApp = () => {
  ...
  const appSlugRouteParam = computed(() => $route.params?.appSlug ? String($route.params.appSlug) : null);
  ...
  const loadClubApp = async (opts: {background?: boolean, clubAppSlug?: string} = {}) => {
