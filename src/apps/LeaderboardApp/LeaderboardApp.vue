  ...
  >
    ...
    >
      ...
      <template v-slot:body-cell-value="props">
        <q-td :props="props" style='font-size: 1rem'>
          <q-badge :class='valueClass(props.row)' :label="props.value" style='font-size: 1rem; height: 1.5rem' />
        </q-td>
      </template>
    </q-table>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const { data: rows, isLoading } = useLoadIndex<ILeaderboardEntry, 'leaderboard'>({
      url: () => props.appData?.clubId && props.appData?.appId ? `/api/club/${props.appData.clubId}/apps/${props.appData.appId}/leaderboard` : null,
      key: 'leaderboard'
    });
