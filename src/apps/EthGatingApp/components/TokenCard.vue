<template>
  <q-card
    :style='{width: size}' style='border-radius: 12px; background-color: rgb(29, 29, 39)' flat
  ...
  <q-dialog v-model='dialogShown'>
    <q-card
      width='280px'
      :style="{borderRadius: '12px', maxWidth: '280px', backgroundColor: themeParams.bg_color, color: themeParams.text_color}"
      class='q-pb-md'
    >
      <q-img
        width='280px'
      ...
      <q-card-section class='text-center q-py-xs q-px-md'>
        <div>
          ...
          <div class='text-left q-pt-sm'>
            <div v-for='(attr, i) in memberToken.metadata.attributes || []' :key='i'>
  ...
  setup(props) {
    ...
    const btnBgColor = computed(() => {
      if (themeParams.value.bg_color && themeParams.value.text_color)
        return blend(changeAlpha(themeParams.value.text_color, 0.048), themeParams.value.bg_color);
      else
        return themeParams.value.bg_color;
    });
