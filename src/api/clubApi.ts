...
export const getClub = async (
  ...
  const response = await api.get<IGetClubResponse>(
    `/api/club/slug/${clubSlug}`,
    {
      headers: {
        'X-SuppressError': '404',
...
export const getMeInClub = async (clubSlug: string) => {
  const response = await api.get<IGetMeInClubResponse>(
    `/api/club/slug/${clubSlug}/me`
  );
  return response.data;
};
...
export const getRoles = async (args: {
  clubId?: number;
  clubSlug?: string;
}) => {
  /* eslint-disable */
  // @ts-ignore
  const clubLocator = args.hasOwnProperty('clubId')
    ? `clubId:${args.clubId}`
    : `clubSlug:${args.clubSlug}`;
  /* eslint-enable */

  const response = await api.get<IGetRolesResponse>(
    `/api/club/${clubLocator}/roles`
  );
...
export const saveAppRoles = async (args: {
  clubSlug: string;
  appSlug: string;
  roles: ISaveRolesRequest_RoleIds;
}) => {
  const response = await api.post<{ ok: boolean }>(
    `/api/club/${args.clubSlug}/app/${args.appSlug}/roles`,
    {
      roles: args.roles,
    }
  );
...
export const saveMemberRoles = async (args: {
  clubSlug: string;
  memberId: string;
  roles: ISaveRolesRequest_RoleIds;
}) => {
  const response = await api.post<{ ok: boolean }>(
    `/api/club/clubSlug:${args.clubSlug}/member/memberId:${args.memberId}/roles`,
    {
      roles: args.roles,
    }
  );
...
export const getBadges = async (args: {
  clubId?: number;
  clubSlug?: string;
}) => {
  /* eslint-disable */
  ...
  const response = await api.get<IGetBadgesResponse>(
    `/api/club/${clubLocator}/badges`
  );
  return response.data.badges;
};

export const assignBadge = async (args: {
  clubSlug: string;
  memberId: string;
  badge: { id: string; value: number };
}) => {
...
export const loadMember = async (args: {
  clubSlug: string;
  memberId: string;
}) => {
  const response = await api.get<ILoadMemberResponse>(
    `/api/club/clubSlug:${args.clubSlug}/member/memberId:${args.memberId}`,
    {}
  );
  return response.data;
};
...
export const loadMe = async (args: { clubSlug: string }) => {
  const response = await api.get<ILoadMemberResponse>(
    `/api/club/clubSlug:${args.clubSlug}/member/me`,
    {}
  );
  return response.data;
};
