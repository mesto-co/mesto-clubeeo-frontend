<template>
  ...
  <div v-if="!isClubLoading" style="padding: 30px 0">
    ...
    <template v-else-if="isLoggedIn">
      ...
      <template v-else>
        ...
        <template
          v-if="club.socialLinks.discord && !club.settings.asideDisableDiscord"
        >
          <club-button
        ...
        <template v-if="club.buyLinks.collections">
          ...
          >
            <club-button class="full-width q-mb-sm" :href="collection.url">
              {{
      ...
      <template
        v-if="meInClub.services.tg.loggedIn && club.settings.whitelistButton"
      >
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const onMintClick = async () => {
      ...
      if (!mintButton.contractAbiUrl)
        return errorNotification('contractAbiUrl is not defined');
      if (!mintButton.signatureUrl)
        return errorNotification('signatureUrl is not defined');
      if (!mintButton.tokenContractAddress)
        return errorNotification('tokenContractAddress is not defined');
      if (!mintButton.chainId)
        return errorNotification('chainId is not defined');
      ...
      if (detectedProvider) {
        ...
        const {
          ...
          .get<{ contractAbi: { abi: AbiItem[] } }>(mintButton.contractAbiUrl)
        ...
        await detectedProvider
          ...
          .catch((e: { code: number }) => {
            if (e.code === 4902) {
              errorNotification(
                `Network #${mintButton.chainId} is not available, please add it to your wallet`
              );
            } else {
        ...
        if (walletAddress !== meInClub.mainWallet.address) {
          return errorNotification(
            "Active wallet account doesn't match your account"
          );
        }

        const { data: signatureData } = await axios
          .get<{
            dt: number;
            token_id: number;
            signature: string;
