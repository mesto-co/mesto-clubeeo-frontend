        ...
        >
          ...
          >
            <template v-slot:after-name>
              <div v-if="badge.clubBadge.badgeType === 'basic'" class='q-pt-sm'>type: basic | index: {{ badge.index}}</div>
              <div v-else-if="badge.clubBadge.badgeType === 'score'" class='q-pt-sm'>type: score | index: {{ badge.index}} | value: {{ badge.value}}</div>
              <div v-else class='q-pt-sm'>type: {{ badge.clubBadge.badgeType }} | index: {{ badge.index}}</div>
            </template>
          </badge-renderer>
        </div>
      </q-card-section>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return {
      ...
      showBadgeAddDialog,
      onBadgeAddClicked: () => showBadgeAddDialog.value = !showBadgeAddDialog.value,
      onBadgeUpdated: () => {
