<template>
  <club-page header="edit club" sticky="bottom" :loading="!club">
    ...
    <template v-if="club">
      <q-form ref="formRef">
        ...
        <div class="text-center">
          <template v-for="socCode in socialLinkCodes" v-bind:key="socCode">
            <div class="q-pb-sm flex flex-center">
              <q-input
                :label="socCode"
                dark
  ...
  setup() {
    ...
    const load = async () => {
      ...
      socialLinkCodes.forEach((code) => {
        if (club.value && !(code in club.value.socialLinks))
          club.value.socialLinks[code as keyof IClubSocialLinks] = '';
      });
    };
    ...
    provide(
      'clubStyle',
      computed(() =>
        Object.assign({}, club.value?.style, {
          socialColor: '#1D1D27',
