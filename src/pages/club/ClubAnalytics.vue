<template>
  ...
  >
    <template v-slot:buttons>
      <q-select
        dense outlined color='primary' dark
    ...
    <div
      style='height: max(50vh, 200px); max-height: max(50vh, 200px)'
    >
      <time-series-chart
        :data='timeSeries'
      />
    ...
    <p>
      total: {{ Object.values(timeSeries).reduce((m, v) => m+v, 0) }}
    </p>

    <p>
      average: {{ Math.round(Object.values(timeSeries).reduce((m, v) => m+v, 0) / Object.values(timeSeries).length * 100) / 100 }}
    </p>
...
export default defineComponent({
  ...
  setup() {
    ...
    const load = async () => {
      ...
      const result = await api.get<{ data: TTimeSeries }>(`/api/club/${slug.value}/dashboard/analytics/${selectedChart.value.value}`);
