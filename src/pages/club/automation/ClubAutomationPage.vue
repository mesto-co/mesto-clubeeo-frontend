<template>
  ...
  >
    <template v-slot:buttons>
      <club-button
        label='create trigger' class='clubButtonActive'
    ...
    >
      ...
      <template v-slot:body-cell-buttons='props'>
        <q-td :props='props'>
          <club-button
            :to='{name: "club-automation-trigger", params: {userId: props.row.id}}'
            dense size='sm'
            class='q-mr-xs'
            icon='fa-solid fa-gear'
            @click='$router.push({name: "club-automation-trigger-edit", params: {triggerId: props.row.id}})'
          >
...
const columns = [
  ...
  { name: 'actionType', align: 'left', label: 'action type', field: 'actionType' },
